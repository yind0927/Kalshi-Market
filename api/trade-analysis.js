...
**格式**：简洁子弹点，每点含¢数和操作指令，总约400字。
${userInput ? `\n用户最新消息：${userInput}\n` : ""}
## 当前局势
- 场面：[真实压制/假性控球/均势] · 信号：[2个具体表现]
- 比分影响：[各标的概率变动方向，一句话]${hasGoal ? "\n- 进球性质：[偶发型/系统型] · 原因：[一句话]" : ""}

## 持仓 × 情景树
结合当前持仓，分析接下来最可能的3种情景：
| 情景 | ${H.cn}Yes价格 | ${A.cn}Yes价格 | 操作建议 |
|------|------|------|------|
| ${H.cn}进球 | +X¢ | −X¢ | [针对持仓的操作] |
| ${A.cn}进球/扳平 | −X¢ | +X¢ | [针对持仓的操作] |
| 继续均势/僵局 | ±X¢ | ±X¢ | [等待信号/微调] |

## 操作决策
- **加仓**：[触发信号] → [标的] ≤X¢ × X单位${isEndGame ? "（禁止）" : ""}
- **止盈**：[标的] 达X¢ → 卖X%${isLateGame ? "；75分钟后只平不追" : ""}
- **止损**：[具体格局改变] → 减至X% 或 全平
- **时间**：${isEndGame ? "仅处理现有仓位，禁止新开" : isLateGame ? "仓位只减不加，60分钟+" : `${min < 25 ? "0-0至25分平局Yes首批可卖；" : ""}继续观察压制信号`}`;
  }
