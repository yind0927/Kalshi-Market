...
- 最大止损：X单位 · 触发：[具体格局改变信号]`;
      ...
      prompt += `
**格式**：简洁子弹点，每点给出可执行操作，约350字。
${userInput ? `\n用户最新消息：${userInput}\n` : ""}
## 分时段情景手册
**0–25分钟（观察+初建仓）**
- 真实压制且0-0 → [标的] 加X单位 ≤X¢；假性控球 → 按兵不动
- 早进球（15分钟内）→ 平局仓立即止损；[进球队]Yes价格变化X¢ → [操作]

**进球后**
- ${stronger}进球偶发型（定位球/折射）→ 价跌时 ≤X¢ 加X单位
- ${stronger}进球系统型（中场失控）→ 持有，不追
- ${weaker}进球 → 偶发减仓50%，系统全平仓

**60分钟后（衰减锁仓）**
- 0-0进入60分钟 → 平局Yes第二批卖出@X¢
- 75分钟起：禁止新开仓，只做止盈止损

## 止盈纪律
- 主方向：达X¢卖50% → 93¢+ 大部分清仓
- 格局已变止损：[${stronger}中场持续失控/被反复打穿/红牌] → 减仓至X%
- 平局Yes：0-0满25分@X¢卖50% → 40分@X¢清仓`;
    }
...
${isEndGame ? "- **禁止**：任何新开仓" : ""}`;
      ...
      prompt += `
当前时段：**${periodName}**${isEndGame ? " ⚠️ 终场：禁止新开仓" : isLateGame ? " ⚠️ 时间衰减：仓位只减不加" : ""}
**格式**：简洁子弹点，每个情景给出触发条件和操作，约300字。

## 比分情景树
| 情景 | 价格冲击 | 操作 |
|------|----------|------|
| ${H.cn}进球 | ${H.cn}Yes +X¢ | [持有/加仓/止盈] |
| ${A.cn}进球/扳平 | ${A.cn}Yes +X¢ | [减仓/平仓] |
| 红牌/点球 | [影响方向] | [操作] |

## 风险预案
- 最大风险：[事件] → 价格冲击X¢ · 应对：[一句话]
- 时间：${isEndGame ? "75分钟+ → 仅平现有仓位" : isLateGame ? "60分钟+ → 仓位只减不加" : "60分后仓位只减，提前设好止盈点"}`;
    }
  }
