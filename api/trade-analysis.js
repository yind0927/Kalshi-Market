...
`;
  ...
  // ── Prematch ──────────────────────────────────────────────
    ...
    prompt += `
**格式**：全部用简洁子弹点，每点含具体¢数或单位数，不写解释段落。总输出约900–1000字。
${userInput ? `\n用户最新消息：${userInput}\n` : ""}
## 速览
- 实力：[Elo差结论，一句话]
- 晋级：[双方晋级形势对打法的影响，一句话]
- 场地/天气：[如无特殊因素请略过]
- 首发风险：赛前需确认 **${H.cn}** / **${A.cn}** 进攻核心是否首发，如有缺阵写一句调整方向
...
| 情景 | 信号 | 操作 |
|------|------|------|
| 0-0 + 真实压制 | 禁区触球↑ / 角球领先 | [标的] 加X单位 ≤X¢ |
| ${stronger}进球（偶发） | 定位球/折射，格局未变 | 价跌时加X单位 |
| ${stronger}进球（系统） | 中场持续失控 | 持有，等第二球 |
| ${weaker}进球 | 先判断性质 | 偶发→减仓50% / 系统→平仓 |
| 60分钟 | 任何情况 | 仓位只减不加 |
| 75分钟+ | 领先/平局 | 分批止盈，禁止新开仓 |
...
**格式**：简洁子弹点，每点含具体¢数或操作，不写解释段落。总输出约700–800字。

## 局势判断
- 比分影响：[三个结果概率相对赛前如何变化，一句话]
- 控场：[真实压制/假性控球/均势] · 依据：[2–3个具体信号]
- 时间阶段：[建仓期/衰减期/锁利润期]
${currentScore && (currentScore.homeScore !== currentScore.awayScore) ? `- 进球性质：[偶发型/系统型] · 理由：[一句话]` : ""}

## 持仓评估
[每笔持仓一行]：标的 @ 入场价 → 现价 · [继续持有/止盈/止损] · 目标价X¢
