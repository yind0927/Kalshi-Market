...
`;
  ...
  // ── Prematch ──────────────────────────────────────────────
    ...
    prompt += `
**输出要求**：前两章（赛事速览 + 首发核查）合计严格不超过150字；后五章（价格纪律起）每章必须详尽，每章不少于120字，不得以"见上"或一句话敷衍。总目标800–1000字。
...
- **75分钟后绝对禁止的3种操作**（具体列出）`;
  ...
  // ── Post-match ─────────────────────────────────────────────
    ...
    prompt += `
请按如下结构输出赛后复盘分析（目标500–700字，每章节详尽）：
...
请按如下结构输出完整实时交易分析（目标700–900字，每个章节必须详尽）：
