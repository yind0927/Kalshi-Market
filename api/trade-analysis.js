...
`;
  ...
  // ── Prematch — single full call ───────────────────────────
  } else if (phase === "prematch") {
    prompt += `
**格式**：全部用简洁子弹点，每点含具体¢数或单位数，不写解释段落。总约700字。
${userInput ? `\n用户最新消息：${userInput}\n` : ""}
## 速览
- 实力：[Elo差结论，一句话]
- 晋级：[双方形势对打法影响，一句话]
...
## 分时段手册
**0–25分钟（观察建仓）**
- 真实压制且0-0 → 主方向加X单位 ≤X¢；假性控球 → 按兵不动
- 平局Yes首批：0-0满25分 + 价格达X¢ → 卖50%
- 早进球（15分内）→ 平局仓立即止损，根据进球性质决定主方向

**进球后**
- ${stronger}进球偶发型（定位球/折射）→ 价跌时 ≤X¢ 加X单位
- ${stronger}进球系统型（中场失控）→ 持有，不追高
- ${weaker}进球 → 偶发：减仓50%；系统：全平仓

**60分钟后（锁利润）**
- 平局Yes第二批@X¢清仓；主方向仓位只止盈不加
- 75分钟起：禁止任何新开仓

## 止盈纪律
- 主方向：达X¢卖50% → 93¢+ 大部分清仓
- 强制止损触发（满足任意一条）：${stronger}中场持续失控 / 被反复打穿边路 / 核心球员红牌
- 禁止：75分钟后情绪追仓 / 信仰补仓 / 因"快赢了"而追高`;
...
${isEndGame ? "- **禁止**：任何新开仓" : ""}`;
      ...
      prompt += `
当前时段：**${periodName}**${isEndGame ? " ⚠️ 终场：禁止新开仓" : isLateGame ? " ⚠️ 时间衰减：仓位只减不加" : ""}
**格式**：简洁子弹点，每个时段/情景给出信号和可执行操作，约500字。
${userInput ? `\n用户最新消息：${userInput}\n` : ""}
## 分时段操作手册
${!isLateGame ? `**当前至${min < 25 ? 25 : 45}分钟**
- 真实压制信号：禁区触球↑ · 角球领先 · 对方只能解围
- 压制明显${!hasGoal ? "且0-0" : ""} → [标的] 加X单位 ≤X¢；假性控球 → 按兵不动
${!hasGoal ? "- 平局Yes首批：0-0满25分@X¢卖50%\n- 早进球 → 平局仓立即止损" : ""}

**进球后**
- ${stronger}进球偶发型 → 价跌时 ≤X¢ 加X单位
- ${stronger}进球系统型 → 持有，不追
- ${weaker}进球 → 偶发减仓50%；系统全平仓
` : ""}**${isEndGame ? "75分钟+（锁仓）" : "60分钟后（衰减）"}**
- ${isEndGame ? "禁止新开仓，仅处理现有仓位" : "仓位只减不加；提前设好止盈触发价"}
- 主方向达93¢ → 大部分清仓；96¢+ 全清

## 比分情景树
| 情景 | 价格冲击 | 操作 |
|------|----------|------|
| ${H.cn}进球 | ${H.cn}Yes +X¢ · 平局Yes −X¢ | [操作] |
| ${A.cn}进球/扳平 | ${A.cn}Yes +X¢ | [操作] |
| 红牌（${H.cn}） | ${H.cn}Yes −X¢ | [减仓/全平] |
| 红牌（${A.cn}） | ${A.cn}Yes −X¢ | [操作] |
| 点球/VAR逆转 | [方向] ±X¢ | [等判罚结果再操作] |

## 风险预案
- 最大尾部风险：[最坏情景] · 冲击X¢ · 应对：[具体一句话]
- 时间红线：${isEndGame ? "已75分钟+，只做现有仓位的止盈止损" : isLateGame ? "60分钟+ → 仓位只减不加" : "60分钟后进入衰减期，提前设好止盈触发点"}`;
    }
  }
