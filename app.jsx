...
/* ─────────────────────────────────────────────────────────
 * Trade Advisor — natural-language multi-factor recommendation
 * Principles:
 *   1. Price > opinion: never chase high prices for stronger team
 *   2. Always keep cash: recommend small positions only
 *   3. Multi-factor: price + time + score + field + counterattack risk
 * ───────────────────────────────────────────────────────── */
function computeTradeAdvice({ M, model, liveModel, market, kalshi, kStatus, live }) {
  const homeProb  = model.probs.home;
  const awayProb  = model.probs.away;
  const favorite  = homeProb >= awayProb ? "home" : "away";
  const underdog  = favorite === "home" ? "away" : "home";
  const favName   = favorite === "home" ? M.home.cn : M.away.cn;
  const undName   = underdog === "home" ? M.home.cn : M.away.cn;
  const favPre    = Math.max(homeProb, awayProb);   // pre-match fav prob
  const favLambda = favorite === "home" ? parseFloat(model.lambdaA) : parseFloat(model.lambdaB);
  const undLambda = underdog === "home" ? parseFloat(model.lambdaA) : parseFloat(model.lambdaB);

  // De-vigged market price for favorite
  const favMkt = favorite === "home" ? market.home : market.away;
  const hasPrice = favMkt != null;

  // ── PRE-MATCH ─────────────────────────────────────────────
  if (!live) {
    if (!hasPrice) {
      return {
        lines: ["尚未接入赔率，无法评估当前价格。请点击"填入 Apply"引入 Pinnacle 赔率，或手动输入种子盘口后再分析。"],
        quality: "wait",
      };
    }
    const edge = favPre - favMkt;  // positive = model > market = underpriced

    if (favPre < 0.52) {
      return {
        lines: [
          `两队实力接近（${M.home.cn} ${(homeProb*100).toFixed(0)}% vs ${M.away.cn} ${(awayProb*100).toFixed(0)}%），价差空间有限。`,
          "实力均衡场次赔率透明、竞争激烈，建议观望，等待比赛展开后再根据场面决策。",
        ],
        quality: "pass",
      };
    }
    ...
    if (edge < -0.05) {
      return {
        lines: [
          `${favName}赔率高于模型估值（市场 ${(favMkt*100).toFixed(0)}% vs 模型 ${(favPre*100).toFixed(0)}%），价格已充分甚至过度定价。`,
          "价格优先原则：即使实力更强，也不在高位追入。等待价格回落到合理区间再考虑。",
        ],
        quality: "pass",
      };
    }
    ...
    if (edge > 0.06) {
      return {
        lines: [
          `${favName}赔率低于模型预期（模型 ${(favPre*100).toFixed(0)}% vs 市场 ${(favMkt*100).toFixed(0)}%），存在可交易的 Edge。`,
          "价格合理，可小仓建仓（不超过 20–25% 资金），保留大部分现金等待比赛展开后再加仓。",
        ],
        quality: "small",
      };
    }
    ...
    return {
      lines: [
        `${favName}赔率与模型基本吻合（边际 ${Math.abs(edge*100).toFixed(0)}pp），价格合理但无明显低估。`,
        "无需主动追入，小仓观察或等待价格变动后再决策。",
      ],
      quality: "observe",
    };
  }
  ...
  // ── IN-PLAY ───────────────────────────────────────────────
  const { scoreA, scoreB, minute } = live;
  const remaining   = Math.max(0, 90 - minute);
  const favScore    = favorite === "home" ? scoreA : scoreB;
  const undScore    = underdog === "home" ? scoreA : scoreB;
  const margin      = Math.abs(favScore - undScore);
  const favLeading  = favScore > undScore;
  const undLeading  = undScore > favScore;
  const tied        = favScore === undScore;

  const isEarly     = minute < 30;
  const isMid       = minute >= 30 && minute < 65;
  const isLate      = minute >= 65;

  // Goal pace: actual vs expected at this minute
  const favExpected = favLambda * (minute / 90);
  const undExpected = undLambda * (minute / 90);

  // "Lucky" if scored significantly more than λ pace suggests
  const favLucky = favScore >= 1 && favScore > favExpected * 1.7;
  const undLucky = undScore >= 1 && undScore > undExpected * 1.7;

  // Edge vs live market
  const liveProbs  = liveModel?.probs;
  const favLivePrb = liveProbs ? (favorite === "home" ? liveProbs.home : liveProbs.away) : null;
  const liveEdge   = (favLivePrb != null && hasPrice) ? favLivePrb - favMkt : null;

  const priceHigh  = liveEdge != null && liveEdge < -0.04;
  const priceFair  = liveEdge != null && Math.abs(liveEdge) <= 0.04;
  const priceLow   = liveEdge != null && liveEdge > 0.05;
  ...
  if (favLeading) {
    if (isLate && margin >= 2) {
      return {
        lines: [
          `${favName}领先 ${favScore}–${undScore}，最后 ${remaining} 分钟 ${undName} 翻盘概率极低。`,
          "应分批止盈，锁定收益，无需继续加仓。",
        ],
        quality: "profit",
      };
    ...
    if (favLucky && isEarly) {
      return {
        lines: [
          `${favName}领先 ${favScore}–${undScore}，但进球节奏超出预期（λ=${favLambda.toFixed(1)}，${minute}' 预计 ${favExpected.toFixed(1)} 球），存在运气成分。`,
          "时间尚早，价格已大幅上涨，${undName}有充足时间反击。不建议高位追入，继续观望。",
        ],
        quality: "pass",
      };
    }
    ...
    if (priceLow) {
      return {
        lines: [
          `${favName}领先且场面主动，价格仍有空间（模型 ${(favLivePrb*100).toFixed(0)}% vs 市场 ${(favMkt*100).toFixed(0)}%）。`,
          "压制明显且价格合理，可小幅加仓；但保留至少 40% 资金，警惕意外进球。",
        ],
        quality: "small",
      };
    }
    ...
    if (priceHigh) {
      return {
        lines: [
          `${favName}领先但市场已过度定价（市场 ${(favMkt*100).toFixed(0)}% 高于模型）。`,
          margin >= 2
            ? "两球领先局势已定，建议分批止盈而非继续追入。"
            : "单球领先尚有变数，高位不追，持有当前仓位观望。",
        ],
        quality: margin >= 2 ? "profit" : "hold",
      };
    }

    return {
      lines: [
        `${favName}领先 ${favScore}–${undScore}，价格合理，局势向好。`,
        isLate
          ? "时间接近尾声，持有至结束即可，无需操作。"
          : `还有 ${remaining} 分钟，维持当前仓位，如价格回落可考虑小幅加仓。`,
      ],
      quality: "hold",
    };
  }
  ...
  // ── Underdog leading ─────────────────────────────────────
  if (undLeading) {
    if (undLucky && favPre > 0.55) {
      return {
        lines: [
          `弱势方 ${undName} 先进球（超出 λ=${undLambda.toFixed(1)} 的预期节奏），领先 ${undScore}–${favScore}。`,
          `${favName}实力占优（赛前 ${(favPre*100).toFixed(0)}%），但弱队领先后通常转入防守，强队虽有翻盘能力但价格已上涨。`,
          "等待强队拉平后价格回落再考虑入场，现在追入性价比低。",
        ],
        quality: "pass",
      };
    }
    ...
    if (isLate && margin >= 2) {
      return {
        lines: [
          `${undName}领先 ${undScore}–${favScore}，仅剩 ${remaining} 分钟，爆冷局面已基本形成。`,
          `继续持有 ${favName}胜方向的仓位风险极高，建议止损。`,
        ],
        quality: "pass",
      };
    }
    ...
    return {
      lines: [
        `${undName}领先，${favName}仍有理论实力优势（赛前 ${(favPre*100).toFixed(0)}%）。`,
        isEarly
          ? "比赛早期，强队有时间反击，但追入价格已涨，性价比有限。暂时观望，等待更明确信号。"
          : `中后段追平难度上升，追入${favName}风险增加，不建议重仓。`,
      ],
      quality: "pass",
    };
  }
  ...
  if (tied) {
    if (isEarly) {
      return {
        lines: [
          "比赛初段，平局属正常开局，场面信号不足。",
          "当前不是最佳入场时机，等待一方建立压制或价格出现明显偏差后再决策。",
        ],
        quality: "wait",
      };
    }

    // Favorite dominant on paper but not converting
    const favDominant = favPre > 0.55 && favLambda > undLambda * 1.3;
    if (favDominant && isMid) {
      return {
        lines: [
          `${favName}应占优势（λ ${favLambda.toFixed(1)} vs ${undLambda.toFixed(1)}）但平局至今，可能只是控球未有效转化为机会。`,
          "不建议仅因实力强就加仓。等待实际压制信号（进球或场面明显提升）再决策。",
        ],
        quality: "pass",
      };
    }
    ...
    if (isLate) {
      return {
        lines: [
          `平局进入最后 ${remaining} 分钟，双方均可能全力一博，波动性极高。`,
          "高风险时段，不建议重仓任何方向；如已有仓位，准备好快速止损或止盈。",
        ],
        quality: "pass",
      };
    }
    ...
    if (priceLow) {
      return {
        lines: [
          `${favName}仍是看好方，当前价格略有低估（模型 ${(favLivePrb*100).toFixed(0)}% vs 市场 ${(favMkt*100).toFixed(0)}%）。`,
          "平局中段，可轻仓跟进（不超过 15–20% 资金），价格若继续回落再加。",
        ],
        quality: "small",
      };
    }
    ...
    return {
      lines: [
        `平局进行中，${favName}理论上占优但比分未体现。`,
        "当前价格与模型接近，无明显机会，继续观望。",
      ],
      quality: "observe",
    };
  }

  return { lines: ["当前数据不足，无法给出具体建议。"], quality: "wait" };
}

function WCTradeAdvisor({ M, model, liveModel, market, kalshi, kStatus, live }) {
  const advice = computeTradeAdvice({ M, model, liveModel, market, kalshi, kStatus, live });
  const meta = {
    small:   { label: "小仓机会", cls: "adv-small" },
    profit:  { label: "止盈信号", cls: "adv-profit" },
    hold:    { label: "持有观望", cls: "adv-hold" },
    pass:    { label: "不建议", cls: "adv-pass" },
    wait:    { label: "等待信号", cls: "adv-wait" },
    observe: { label: "观察中",   cls: "adv-observe" },
  }[advice.quality] || { label: "—", cls: "adv-wait" };

  return (
    <div className={`card wc-card wc-advisor ${meta.cls}`}>
      <div className="wc-adv-head">
        <h3 className="wc-adv-title">交易建议 <em>Trade Advisor</em></h3>
        <span className={`wc-adv-pill ${meta.cls}`}>{meta.label}</span>
      </div>
      {advice.lines.map((line, i) => (
        <p key={i} className={`wc-adv-line${i === 0 ? " lead" : ""}`}>{line}</p>
      ))}
      <div className="wc-adv-rule">
        价格优先 · 始终保留现金 · 综合 价格 / 时间 / 比分 / 场面 后决策
      </div>
    </div>
  );
}
...
function WorldCupView({ resultOverrides = {} }) {
  ...
  return (
    <div className="view wc-view" data-screen-label="worldcup">
      ...
      {/* ── Trade Advisor (hidden for finished matches) ── */}
      {!showFinishedView && (
        <WCTradeAdvisor
          M={M}
          model={model}
      ...
      <div className="risk-card" style={{ marginTop: 0 }}>
